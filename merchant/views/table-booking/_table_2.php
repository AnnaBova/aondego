...
<?php echo $this->registerJs("
    ...
    //console.log($('.calendar-".$model->id."'));
        ...
        $('.calendar-".$model->id."').fullCalendar({
	     ...
	     var tooltip = '<div class=\"tooltipevent\" style=\"width:200px;max-height:100%;color:#fff;background:#167a7b;padding:10px;border:solid 1px #2a4e54;position:absolute;z-index:10001;\"> <small>' + calEvent.description + '</small></div>'; 
                ...
                if (event.url) {
                    if(event.url.indexOf('update-inst')>0) {
						...
						$('#client-type').hide();
							...
							$('#free_time_list-dis').html(data.selectedtime);
							console.log('i mahere at update');
							console.log($('#free_time_list-dis'));
