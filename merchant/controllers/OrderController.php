    ...
    public function actionAdd()
    {
		...
		if($model->validate()){
			...
			if($selectedClient == 0){
				...
				if($client->validate()){
					$client->save(false);
					\merchant\components\EmailManager::customerAccountActivate($client, Yii::$app->user->id);
			...
			if ($model->save()) {
				$model = $this->loadModelSingle($model->id);
...
//                    ->all();
    ...
    public function actionUpdateInst($id)
    {
        ...
        $time = date('H:i', strtotime($model->order_time));
		
        $name = '<a style="background:#167a7b;" class="btn_4 select-free-time" href="javascript:void(0)" data-name="'.$time.'">'.$time.'</a>';;
		$dd = Html::tag('li',($name));
