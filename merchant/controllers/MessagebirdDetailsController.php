...
//         print_r(Yii::$app->user->identity->merchant_id);
        ...
        if ($model->load(Yii::$app->request->post())){
            ...
            $mid=$_POST['MessagebirdDetails']['merchant_id'];
            $enable= $_POST['MessagebirdDetails']['enable'];
            $type= $_POST['MessagebirdDetails']['sms_type'];
            if(isset($_POST['MessagebirdDetails']['enable'])){
              
                $merchant= Merchant::find()->where(['merchant_id'=>$mid])->one();
                $merchant->enable_sms= 1;
