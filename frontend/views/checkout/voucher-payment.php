...
<div id="position">
    <div class="container">
        <ul>
            ...
            <li>
                <a href="<?php echo Yii::$app->urlManager->createUrl(['merchant/gift-voucher', 'id' => $merchantUrl]) ?>">
		...
		<div class="box_style_2">
			...
			if (in_array(0, $merchant->giftVoucherSetting->delivery_options)) { ?>
				...
				<label>
				    ...
				    <input type="radio" value="0" name="AddToCart[deliveryOption]" class="icheck delivery-option">
				    <?php echo Yii::t('basicfield', 'Pick up')?>
		...
		<div id="cart_box">
			...
			<tbody>
			    ...
			    if(in_array(1, $merchant->giftVoucherSetting->delivery_options)){
				    ...
				    echo $form->field($addToCart, 'deliveryFee')->hiddenInput(['value'=> $merchant->giftVoucherSetting->delivery_fee])->label(false);
				    ?>
			    <tr>
				<td>
...
$paymentUrl  = Yii::$app->urlManager->createUrl('checkout/voucher-payment');
$js = <<<SCRIPT
	...
	$('body').on('click','#proceed-checkout', function(e){
		e.preventDefault();
		$('.loading').show();
		var obj = $(this);
