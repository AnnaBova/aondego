...
Class WidgetController extends \yii\web\Controller{
	...
	public function actionGiftVoucher($id){
		...
		$merchant = \frontend\models\MtMerchant::findOne(['merchant_id' => $id]);
    ...
    //                        exit;
				...
				$email = \frontend\components\EmailManager::customerAccountActivate($client);
				$url = Yii::$app->urlManager->createUrl('/widget/verification');
	...
	public function actionVerification(){
		...
		if($client->load(Yii::$app->request->post())){
		    if($client->validate()){
			$checkClient = \frontend\models\Client::findOne(['activation_key' => trim($client->activation_key)]);
