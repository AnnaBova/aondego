...
use Yii;
use Addon;
use frontend\components\UrlHelper;
//use LoyaltyPoints;
use frontend\models\MerchantSchedule;
use frontend\models\MerchantScheduleHistory;
use frontend\models\ScheduleDaysTemplate;
...
class MtMerchant extends ActiveRecord implements IdentityInterface
{
    ...
    public function rules()
    {
        return [
            ...
            [['confirm_password', 'password_hash', 'password_reset_token',
		'auth_key', 'language_id', 
		'currency_id'
		], 'safe'],
            [['contact_email'], 'email'],
	...
	public static function hasServices($merchant){
		$categoryHasMerchant= CategoryHasMerchant::find()->where(['merchant_id' => $merchant->merchant_id])->count();
		
		return $categoryHasMerchant;
		
	}
	
	public static function hasVoucher($merchant){
		$giftVoucher= \common\models\GiftVoucher::find()->where(['merchant_id' => $merchant->merchant_id])->count();
	...
	public static function getMerchantCountByCities($city){

		$sql = 'SELECT count(*) as total ,LCASE(city) as city from mt_merchant where city="'.$city.'" and status=1 and is_activate=1';
