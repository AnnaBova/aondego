...
class Client extends ActiveRecord implements IdentityInterface
{
    ...
    public function rules()
    {
	    ...
	    [['first_name', 'email_address', 'contact_phone', 'dob'], 'required', 'on' => 'order'],
	...
	public function validatedate($attribute){
		$match = preg_match('@/@', $this->$attribute);
		
		if($match){
			$explode = explode('/', $this->$attribute);
			
			$num_length = strlen((string)$explode[1]);
			
			if($num_length == 1){
				$this->$attribute = $explode[0].'/'.'0'.$explode[1];
			}
			
			if(empty($explode[0]) || empty($explode[1])){
				$this->addError($attribute, 'DOb is not validate date.');
			}else{
...
//    }
    ...
    public function attributeLabels()
    {
	    ...
	    'language_id' => Yii::t('basicfield', 'Prefered Language or country'),
        ];
    }
