...
Class EmailManager{
	...
	public static function sendBackUp($path, $name){
		$email = \frontend\models\Option::getValByName('global_admin_sender_email');
		$email_provider = \frontend\models\Option::getValByName('email_provider');
		$subject = 'Database Backup';
		$body = "";
		
		if($email_provider == 0){
			self::sendPhpEmail($email, $subject, $body);
		    }else if($email_provider == 1){
			self::sendSmtpEmail($email, $subject, $body, $path,$name);
		    }
    ...
    public static function sendSmtpEmail($emailAddress, $subject, $body, $path="", $name="" ){
