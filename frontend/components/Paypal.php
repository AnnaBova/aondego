...
Class Paypal{
	...
	if($this->is_sandbox){    
		...
		$this->paypal_refund_url = 'https://www.sandbox.paypal.com/cgi-bin/webscr';
	}else{
		$this->paypal_refund_url = 'https://www.paypal.com/cgi-bin/webscr';
	}
    }
...
//                        exit;
	...
	//            $this->log_ipn_results(false);
		...
		//if (eregi("VERIFIED", $this->ipn_response)) {
			...
			$sql = 'INSERT into dummy (dummy) values("'.  $custom.'")';
			$query = Yii::$app->db->createCommand($sql)->execute();

			if(!empty($custom)){
				
				$sql = 'INSERT into dummy (dummy) values("'.  $post['txn_id'].'")';
				$query = Yii::$app->db->createCommand($sql)->execute();

				$order = \common\models\Order::findOne(['id' => $custom]);
				$order->status = 0;
				$order->txn_id = $post['txn_id'];
				$order->payment_status = $post['payment_status'];

				if($order->save(false)){
					
					EmailManager::buyVoucher($order);
					EmailManager::buyVoucherMerchant($order);

					$orders = \common\models\Order::findAll(['order_id' => $order->id]);
					...
					foreach ($orders as $data){
						...
						EmailManager::buyVoucher($data);
						EmailManager::buyVoucherMerchant($data);
    ...
    public static function Post($session){
        ...
        if(!empty($session['cart'])){
            ...
            $host = ($server == TRUE) ? Yii::$app->getRequest()->getHostInfo() : 'http://c0e02485.ngrok.io/appointment-portal';
...
//                exit;
	...
	public static function VoucherPost($session){
		...
		if(!empty($session['Vouchercart'])){
			...
			$host = ($server == TRUE) ? Yii::$app->getRequest()->getHostInfo() : 'http://881bc04b.ngrok.io/appointment-portal';


			$query['notify_url'] = $host.'/checkout/voucher-paypal-ipn';
			$query['return'] = $host.'/checkout/voucher-finish';
			$query['cancel_return'] = $host.'/checkout/voucher-payment';
			$query['cmd'] = '_cart';
			$query['upload'] = '1';
			$query['business'] = $merchant->paypall_id;
			$query['address_override'] = '0';
