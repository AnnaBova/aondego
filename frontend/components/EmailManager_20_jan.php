...
Class EmailManager{
	...
	public static function buyVoucher($order){
		...
		$variable['voucher_message'] = $order->merchant->service_name;
	...
	public static function buyVoucherMerchant(){
		$language = $order->merchant->language->code;
        
		$option = \frontend\models\Option::getValByName('email_tpl_sub_merchant_voucher_order', $language);
...
//        print_r($body);
		...
		if($merchant->enable_sms==1 && ($client->notification==1||$client->notification==2)){
			MessagebirdManager::sendsmsbirthday($user, $coupon, $userBirthdayCoupon);
		}
