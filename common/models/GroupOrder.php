...
class GroupOrder extends Order
{
    ...
    public function rules()
    {
        ...
        return [
            [['client_name', 'client_phone', 'client_email', 'order_time', 'category_id', 'no_of_seats'], 'required'],
            ...
            [['price', 'no_of_seats'], 'integer'],
            ['no_of_seats', 'checkSeats', 'skipOnEmpty' => false, 'skipOnError' => false],
    ...
    public function checkSeats($attribute, $params)
    {
        ...
        $category = CategoryHasMerchant::findOne(['id' => $this['category_id']]);
        //print_r($category->group_people);
        $left = GroupOrder::countByDate($this['order_time'], $this['category_id']);
        
        if(isset($this['id'])){
            $sql = 'SELECT SUM(no_of_seats) as total from `order` where order_time="'.$this['order_time'].'" and category_id="'.$this['category_id'].'" and status=0 and id !='.$this['id'];
