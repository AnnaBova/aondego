    ...
    public static function saveMessage($model, $field, $language){
        ...
        foreach ($field as $key=>$value){
            if(!empty($model->$value)){
                ...
                if(count($message) !=0 && isset($result1['id'])){
...
//                    $command->execute();
                    ...
                    $messageUpdate = \common\models\Message::findOne(['id' => $result1['id'], 'language'=>trim($language) ]);
                    $messageUpdate->translation = $model->$value;
                    $messageUpdate->save(false);
    ...
    public static function getLanguage($model=null){
        ...
        $i=1;foreach ($language as $data){
            ...
            if($category == 'option'){
                ...
                echo '<li class="'.$class.'"><a href="'.Yii::$app->urlManager->createUrl([$controller.'/'.$action,'language'=>$data->code]).'" >'.$data->name.'</a>';
            }else{
