...
Class WidgetController extends \yii\web\Controller{
	...
	public function actionVoucherPayment(){
		...
		$address = new \fbwidget\models\MtAddressBook;
		$address->scenario = 'giftvoucher';
				...
				if(!empty($session['Vouchercart'])){
					...
					if($addToCart->deliveryOption == 2){
						...
						if($address->validate()){
							...
							echo Json::encode(['success' => false, 'message' => $address->errors]);
							Yii::$app->end();

						}

					}else if($addToCart->deliveryOption == 1){
						$addressId = \fbwidget\models\MtAddressBook::findOne(['client_id' => Yii::$app->user->id, 'as_default' => 1])->id;
					}
