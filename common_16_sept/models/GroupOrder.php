...
class GroupOrder extends Order
{
    ...
    public function rules()
    {
        ...
        return array(
            array('client_name, client_phone, client_email, order_time, category_id', 'required'),
            array('status, staff_id, client_id, payment_type, category_id, status, merchant_id', 'numerical', 'integerOnly' => true),
            array('client_name, client_phone, client_email', 'length', 'max' => 50),
            ['order_time, addons_list,g_cache', 'safe'],
            array('more_info', 'length', 'max'=>510),
    ...
    public static function countByDate($date, $service_id){
        return self::model()->countByAttributes(['order_time'=>$date,'category_id'=>$service_id,'status'=>0])+CachedSingleOrder::countGroupByDay($service_id,$date);
    }
