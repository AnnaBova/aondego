 ...
 */
class StaffSchedule extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			...
			array('status, schedule_days_template_id, staff_id', 'numerical', 'integerOnly'=>true),
            ['reason','length', 'max'=>255],
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, work_date, status, schedule_days_template_id, staff_id', 'safe', 'on'=>'search'),
		);
	}
    ...
    public function afterSave(){
        ...
        $condition->condition = 'staff_id = '.$this->staff_id.' AND order_time>= "'.$this->work_date.' 00:00:00'.'" AND order_time<="'.$this->work_date . ' 23:59:59'.'"';
        $orders = SingleOrder::model()->findAll($condition);
        foreach($orders as $order){
            $setFree = true;
            if($this->scheduleDaysTemplate){
                foreach($this->scheduleDaysTemplate->timeRanges as $val){
                    if(($order->order_time>=$this->work_date.' '.$val->time_from.':00')&&($order->order_time<$this->work_date.' '.$val->time_to.':00')&&
                        (date('Y-m-d H:i:s',strtotime("+{$order->getOrderTimeLength()}",strtotime($order->order_time)))>$this->work_date.' '.$val->time_from.':00')&&(date('Y-m-d H:i:s',strtotime("+{$order->getOrderTimeLength()}",strtotime($order->order_time)))<=$this->work_date.' '.$val->time_to.':00')){
                        $setFree = false;
                        break;
                    }
                }
