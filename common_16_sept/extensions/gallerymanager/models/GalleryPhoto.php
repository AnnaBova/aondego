...
//            array('gallery_id, rank', 'numerical', 'integerOnly' => true),
    ...
    private function getPrefix(){
        ...
        $this->_prefix .= '/frontend/www';
        return $this->_prefix;
    }

    public function getPreview()
    {
        return $this->getPrefix().'/'. $this->galleryDir . '/_' . $this->getFileName('') . '.' . $this->galleryExt;
    }
    ...
    public function setImage($path)
    {
        ...
        Yii::app()->image->load($path)->save(Yii::getPathOfAlias('site').'/frontend/www/' .$this->galleryDir . '/' . $this->getFileName('') . '.' . $this->galleryExt);
        //create image preview for gallery manager
        Yii::app()->image->load($path)->resize(300, null)->save(Yii::getPathOfAlias('site').'/frontend/www/' .$this->galleryDir . '/_' . $this->getFileName('') . '.' . $this->galleryExt);
    ...
    {
        $this->removeFile(Yii::getPathOfAlias('site').'/frontend/www/' . $this->galleryDir . '/' . $this->getFileName('') . '.' . $this->galleryExt);
        $this->removeFile(Yii::getPathOfAlias('site').'/frontend/www/'  . $this->galleryDir . '/_' . $this->getFileName('') . '.' . $this->galleryExt);
    ...
    public function removeImages()
    {
        foreach ($this->gallery->versions as $version => $actions) {
            $this->removeFile(Yii::getPathOfAlias('site').'/frontend/www/'  .$this->galleryDir . '/' . $this->getFileName($version) . '.' . $this->galleryExt);
        }
    }
    ...
    public function updateImages()
    {
        foreach ($this->gallery->versions as $version => $actions) {
            ...
            $image = Yii::app()->image->load(Yii::getPathOfAlias('site').'/frontend/www/'  .$this->galleryDir . '/' . $this->getFileName('') . '.' . $this->galleryExt);
            foreach ($actions as $method => $args) {
                call_user_func_array(array($image, $method), is_array($args) ? $args : array($args));
            }
            $image->save(Yii::getPathOfAlias('site').'/frontend/www/'  .$this->galleryDir . '/' . $this->getFileName($version) . '.' . $this->galleryExt);
        }
    }
